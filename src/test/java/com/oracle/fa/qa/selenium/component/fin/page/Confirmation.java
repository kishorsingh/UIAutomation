...
public class Confirmation extends BasePageObject<Confirmation>{
	...
	@FindBy(css = "button[id*='OKSubmitDialogButton']")
	WebElement ok;


	@FindBy(xpath = "//a[text()='Print Report']")
...
//	        }else
	        	if(PageLoadHelper.waitForElementClickable(printReport) == null ){
			 throw new TestErrorException("The Print Report field is not visible after " +
					 Timeout.pageLoadValue().seconds() + " seconds.");
		 	}
		 	StepReport.info("Confirmation Page");
	    }
	 ...
	 public TravelandExpenses clickOk(){
		...
		if(!printReport_message){
			PageLoadHelper.waitForElementClickable(ok);
			ok.click();
		}
